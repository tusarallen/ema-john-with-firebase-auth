/**
 * 1.create a firebase project
 * 2.create web app
 * 3.npm i firebase
 * 4.save firebase config and export default app
 * 5.Build > Authentication > Get Started > Enabled > Sign In Methode
 * 6.create sign up and Login route
 * */


/**
 *                       CONTEXT API
 * 1.create a context provider (folder with file)
 * 2.create a context and set provider
 * 3.set the children props
 * 4.set context value
 * 5.set the provider
 * */ 

/**
 * HOSTING
 * ------------------
 * 1.one time per pc
 * 2.npm install -g firebase-tools
 * 3.firebase login
 * -------------------
 * 
 * For each project one time
 * 1.firebase init
 * 2.proceed
 * 3.hosting: firebase (up and down arrow) click space bar to select
 * 4.existing project
 * 5.select the project cearfully
 * 6.which project as public directory: dist
 * 7.single page application: yes
 * 8.continious deployment: no
 * ---------------------------
 * For every time deploy
 * 1.npm run build
 * 2.firebase deploy
 * */ 